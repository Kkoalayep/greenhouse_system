...
#include "delay.h"
#include <stdarg.h>
uint8_t MyBuffer[2048]={0};
uint8_t MyReciveSta=0;
uint16_t MyReciveLenth=0;

void usart2_Init(u32 bound)//串口2  引脚为PA2  PA3
{
...
/*bool ESP8266_MQTTPUB_ME( char  pAXm, char  pAYm, char  pBXm, char  pBYm, char  pCXm, char  pCYm, char  pDXm )
{
    char cCmd [300];
    sprintf ( cCmd, "AT+MQTTPUB=0,\"/sys/post\",\"{\\\"id\\\":\\,\\\"params\\\":{\\\"temperature\\\":%s\\,\\\"Humidity\\\":%s\\,\\\"illumination\\\":%s\\,\\\"co2\\\":%s\\,\\\"TVOC\\\":%s\\,\\\"BMP_Pressure\\\":%s\\,\\\"BMP_Temperature\\\":%s}\\,\\\"version\\\":\\\"1.0\\\"\\,\\\"method\\\":\\\"thing.event.property.post\\\"}\",1,0", pAXm,pAYm,pBXm,pBYm,pCXm,pCYm,pDXm );
    return ESP8266_Send_AT_Cmd( cCmd, "OK", NULL, 500 );
}*/
