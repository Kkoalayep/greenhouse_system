...
//IO方向设置
#define BMP_SDA_IN()  {GPIOD->MODER&=~(3<<(1*2));GPIOD->MODER|=0<<1*2;}	//PD1输入模式
#define BMP_SDA_OUT() {GPIOD->MODER&=~(3<<(1*2));GPIOD->MODER|=1<<1*2;} //PD1输出模式
//IO操作函数	 
#define BMP_IIC_SCL    PDout(0) //SCL
#define BMP_IIC_SDA    PDout(1) //SDA	 
#define BMP_READ_SDA   PDin(1)  //输入SDA 
